...
import { Plus, Edit, Trash2, Tag, X, Sparkles, ImagePlus, ArrowRight, Layers, LayoutGrid, Zap, Award } from 'lucide-react';
    ...
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white/95 backdrop-blur-2xl rounded-[2.5rem] border border-white shadow-[0_32px_64px_-16px_rgba(0,0,0,0.2)] w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-300">
        ...
        <div className="p-8 text-center">
          ...
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.4em] mb-2 italic text-[10px]">Hệ thống PawVerse</p>
...
export default function BrandManagementPage() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success('THƯƠNG HIỆU ĐÃ ĐƯỢC NIÊM YẾT!', {
        icon: '🏷️',
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(['admin-brands']);
      toast.success('ĐÃ XÓA THƯƠNG HIỆU KHỎI HỆ THỐNG!', {
        icon: '🗑️',
        style: { borderRadius: '15px', background: '#111', color: '#fff', fontSize: '11px', fontWeight: 'bold' }
      });
    },
    onError: () => toast.error('Không thể xóa thương hiệu do liên kết dữ liệu'),
  });
  ...
  const handleDelete = (brand) => {
    setConfirmModal({
      ...
      message: `Bạn có chắc chắn muốn xóa thương hiệu “${brand.tenBrand}”? Hành động này sẽ gây lỗi nếu thương hiệu có sản phẩm đi kèm.`,
      type: 'danger',
      onConfirm: () => deleteMutation.mutate(brand.idBrand)
    });
  };
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-4">
         ...
         <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest animate-pulse italic">Đang tải dữ liệu thương hiệu...</p>
      </div>
    );
  }
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em] italic">Nhãn hiệu & Đối tác</span>
          </div>
          <h1 className="text-4xl font-black text-gray-950 uppercase tracking-tighter italic leading-none">
            Quản lý <span className="text-orange-500 font-black not-italic">Thương hiệu</span>
          </h1>
          <p className="text-gray-500 mt-3 font-medium text-sm flex items-center gap-2">
            <Layers size={14} className="text-gray-300" />
            Đang hợp tác với <span className="text-gray-950 font-black italic underline decoration-orange-200">{brands?.length ?? 0}</span> nhãn hàng quốc tế & nội địa
          </p>
        </div>
        ...
        >
          ...
          <span className="text-[11px] font-black uppercase tracking-[0.1em] italic">Thêm thương hiệu mới</span>
        </button>
      </div>

      {/* Brands Grid - Optimized PRO MAX */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {brands?.map((brand) => (
          <div key={brand.idBrand} className="group bg-white rounded-[2.5rem] border border-gray-100 p-6 hover:shadow-[0_32px_64px_-16px_rgba(0,0,0,0.06)] hover:border-orange-100 transition-all duration-500 relative overflow-hidden">
            <div className="absolute -right-4 -top-4 text-orange-50 opacity-0 group-hover:opacity-100 group-hover:scale-110 transition-all duration-700 rotate-12 pointer-events-none">
               <Award size={160} />
            </div>

            <div className="flex items-start justify-between mb-6 relative z-10">
              <div className="w-16 h-16 bg-gray-50 rounded-[1.5rem] flex items-center justify-center overflow-hidden border border-gray-100 group-hover:border-orange-200 group-hover:rotate-3 transition-all duration-500 shadow-inner">
                {brand.logo ? (
                  <img src={brand.logo} alt={brand.tenBrand} width={64} height={64} className="w-full h-full object-cover" />
                ) : (
                  <Tag className="text-gray-300 group-hover:text-orange-500 transition-colors" size={28} />
                )}
              </div>
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-2 mb-2">
                <h3 className="text-[17px] font-black text-gray-900 uppercase tracking-tight group-hover:text-orange-600 transition-colors italic leading-tight">{brand.tenBrand}</h3>
                <span className={`text-[9px] px-2 py-0.5 rounded-lg font-black uppercase tracking-widest italic ${
                  brand.trangThai === 'Hoạt động' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'
                }`}>
                  {brand.trangThai}
                </span>
              </div>
              <p className="text-gray-400 text-[11px] font-medium italic line-clamp-2 min-h-[32px] leading-relaxed">
                {brand.moTa || 'Thương hiệu đối tác chiến lược của PawVerse.'}
              </p>
              
              <div className="mt-6 pt-5 border-t border-gray-50 flex items-center justify-between">
                <div className="flex items-center gap-1.5">
                   <Tag size={14} className="text-gray-300 font-black italic" />
                   <span className="text-[11px] font-black text-gray-900 italic font-variant-numeric: tabular-nums">
                     {brand.productCount || 0} <span className="text-[10px] text-gray-400 uppercase font-bold not-italic ml-0.5 tracking-tighter">Lineups</span>
                   </span>
                </div>
                <div className="text-[9px] font-black text-gray-300 uppercase italic tracking-widest">UID: {brand.idBrand}</div>
              </div>
            </div>
          </div>
        ))}
      ...
      {brands?.length === 0 && (
        <div className="bg-white/60 backdrop-blur-xl rounded-[3rem] border border-gray-100 py-32 text-center shadow-sm">
          ...
          <p className="text-[11px] font-black text-gray-400 uppercase tracking-[0.3em] italic leading-none">Cơ sở dữ liệu thương hiệu trống</p>
        </div>
      )}
      ...
      {showModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white/95 backdrop-blur-2xl rounded-[3rem] border border-white shadow-[0_40px_120px_-20px_rgba(0,0,0,0.3)] w-full max-w-lg overflow-hidden flex flex-col animate-in slide-in-from-bottom-12 duration-700">
            ...
            <div className="flex items-center justify-between px-8 py-6 shrink-0 bg-gradient-to-b from-white/50 to-transparent">
              <div>
                 <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-[8px] font-black text-gray-400 uppercase tracking-[0.3em] italic leading-none">Thông số đối tác</span>
                 </div>
                 <h2 className="text-2xl font-black text-gray-950 uppercase tracking-tighter italic leading-none">
                   {editingBrand ? 'Chỉnh sửa' : 'Niêm yết'} <span className="text-orange-500 font-black not-italic">Thương hiệu</span>
                 </h2>
              </div>
              <button 
            ...
            <form onSubmit={handleSubmit} className="p-8 pt-2 space-y-6 overflow-y-auto max-h-[70vh] custom-scrollbar italic font-black">
              <div className="space-y-5">
                <div className="group font-black uppercase italic">
                  <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest italic mb-1.5 px-1 font-black">Tên thương hiệu <span className="text-orange-500 font-black italic">*</span></label>
                  <input
                    type="text"
                    name="tenBrand"
                    ...
                    required
                    placeholder="VD: Reflex, Royal Canin, Minino..."
                    className="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:bg-white focus-visible:ring-4 focus-visible:ring-orange-100 outline-none transition-all text-sm font-black text-gray-950 placeholder:font-medium placeholder:italic placeholder:text-gray-300 italic uppercase"
                  />
                </div>

                <div className="group font-black uppercase italic">
                  <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest italic mb-1.5 px-1 font-black italic uppercase">Mô tả đối tác</label>
                  <textarea
                    name="moTa"
                    value={formData.moTa}
                ...
                <div className="group">
                  <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest italic mb-1.5 px-1 font-black uppercase italic">Logo đại diện (URL)</label>
                  <div className="flex gap-3">
                    <div className="flex-1 relative">
                      <input
                        ...
                        placeholder="Link ảnh logo nhãn hàng..."
                        className="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:bg-white focus-visible:ring-4 focus-visible:ring-orange-100 outline-none transition-all text-[11px] font-medium italic"
                      />
                    </div>
                    <div className="w-14 h-14 bg-gray-50 rounded-2xl border border-gray-100 flex items-center justify-center shrink-0 overflow-hidden shadow-inner font-black uppercase italic">
                      {formData.logo ? (
                        <img src={formData.logo} alt="" width={56} height={56} className="w-full h-full object-cover" />
                      ) : (
                ...
                <div className="group font-black uppercase italic">
                  <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest italic mb-1.5 px-1">Tình trạng hợp tác</label>
                  <select
                    name="trangThai"
                  ...
                  >
                    <option value="Hoạt động">ĐANG CUNG ỨNG HÀNG</option>
                    <option value="Ngưng hoạt động">TẠM NGỪNG NHẬP HÀNG</option>
                  </select>
                </div>
              </div>
