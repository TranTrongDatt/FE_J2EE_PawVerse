...
→ Backend chưa chạy. Chạy backend trước, đợi thấy "Started PawVerseApplication" rồi mới mở frontend.
...
### ❌ reCAPTCHA không hiện
→ Kiểm tra file `.env` có `VITE_RECAPTCHA_SITE_KEY` chưa. Sau khi sửa `.env` phải **restart Vite** (tắt và chạy lại `npx vite`).

### ❌ Java version không đúng
```bash
java -version
# Phải là 21+. Nếu chưa đúng, cài JDK 21 và set JAVA_HOME.
```
